<template>
    <b-card no-body>
        <div class="m-2">
            <b-row>
                <b-col cols="12" md="8" class="d-flex align-items-center justify-content-start mb-1 mb-md-0">
                    ...
                    <b-form-input v-model="filterName" class="d-inline-block mr-50 advance-search" :placeholder="$t('Name')" />
    
                    <b-form-input v-model="filterCity" class="d-inline-block mr-50 advance-search" :placeholder="$t('City')" />
                ...
                <b-col>
                    <div class="d-flex align-items-center justify-content-end">
                        <b-form-input v-model="filter" class="d-inline-block mr-50 advance-search" :placeholder="$t('Advance search')" />
                    </div>
                </b-col>
            </b-row>
            <b-row>
                ...
                <b-col cols="12" md="6" class="d-flex align-items-center justify-content-center mb-1 mb-md-0">
                    <b-pagination v-model="currentPage" :total-rows="totalInvoices" :per-page="perPage" first-number last-number class="mb-0 mt-1 float-right mr-1" prev-class="prev-item" next-class="next-item">
                        <template #prev-text>
            ...
            <b-row>
                <b-col cols="12" md="12" class="d-flex align-items-center justify-content-start mb-1 mb-md-0 mt-1">
                    <b-button class="mr-1" v-ripple.400="'rgba(113, 102, 240, 0.15)'" variant="outline-primary" id="PR" @click="multipleAction">
                        {{ $t("Print") }}
                    </b-button>
    
                    <b-button class="mr-1" v-ripple.400="'rgba(113, 102, 240, 0.15)'" variant="outline-primary" id="SW" @click="multipleAction">
                        {{ $t("send-to-warehouse") }}
                    </b-button>
    
                    <b-button class="mr-1" v-ripple.400="'rgba(113, 102, 240, 0.15)'" variant="outline-primary" id="PU" @click="multipleAction">
                        {{ $t("picked-up") }}
                    </b-button>
    
                    <b-button class="mr-1" v-ripple.400="'rgba(113, 102, 240, 0.15)'" variant="outline-primary" id="SI" @click="multipleAction">
                        {{ $t("send to invoice") }}
                    </b-button>
    
                    <b-button class="mr-1" v-ripple.400="'rgba(113, 102, 240, 0.15)'" variant="outline-primary" id="DL" @click="multipleAction">
                        {{ $t("Delete") }}
                    </b-button>
                </b-col>
            </b-row>
        ...
        :tbody-tr-class="Cell_table_class"
        >
            <div>
            </div>
            <template #head(checked_id)>
                <input type="checkbox" @click="checkallOrder($event)" name="order_checkedall[]" class="check-btn">
            </template>
    
            <!-- Column: Id -->
            <template #cell(checked_id)="data" class="check-main">
                <div v-if="(data.item.DEL == 'Y')" class="deleted-row">
                    <input type="checkbox" @click="checkOrder($event)" name="order_checked[]" class="check-btn" :value="data.item.orderNumber" :id="data.item.orderNumber" disabled>
                </div>
                <div v-else>
                    <input type="checkbox" @click="checkOrder($event)" name="order_checked[]" class="check-btn" :value="data.item.orderNumber" :id="data.item.orderNumber" :checked="checkall">
                </div>
            </template>
            ...
            <template #cell(PRN,DSP,INV,DEL)="data">
                ...
                <span v-if="(data.item.PRN == 'Q')" style="margin-left: -12px;">
                    <span data-v-9a6e255c="" class="b-avatar rounded-circle" style="width: 32px; height: 32px;  background-color:blue;" id="invoice-row-5027">
                ...
                <span v-if="(data.item.PRN == 'R')" style="margin-left: -12px;">
                    <span data-v-9a6e255c="" class="b-avatar badge-light-warning rounded-circle" style="width: 32px; height: 32px; " id="invoice-row-5027">
                        <span class="b-avatar-custom">
                            <svg xmlns="http://www.w3.org/2000/svg" width="14px" height="14px" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-file">
                                <path d="M13 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V9z"></path>
                                <polyline points="13 2 13 9 20 9"></polyline>
                ...
                <span v-if="(data.item.PRN == undefined)" style="margin-left: -12px;">
                    <span data-v-9a6e255c="" class="b-avatar badge-light-secondary rounded-circle" style="width: 32px; height: 32px; " id="invoice-row-5027">
                        <span class="b-avatar-custom">
                            <svg xmlns="http://www.w3.org/2000/svg" width="14px" height="14px" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-file">
                                <path d="M13 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V9z"></path>
                                <polyline points="13 2 13 9 20 9"></polyline>
                ...
                <span v-if="(data.item.PRN == 'N')" style="margin-left: -12px;" title="printed">
                    <span data-v-9a6e255c="" class="b-avatar rounded-circle" style="width: 32px; height: 32px; background-color: lightblue;" id="invoice-row-5027">
                        <span class="b-avatar-custom">
                            <svg xmlns="http://www.w3.org/2000/svg" width="14px" height="14px" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-file">
                                <path d="M13 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V9z"></path>
                                <polyline points="13 2 13 9 20 9"></polyline>
                ...
                <span v-if="(data.item.PRN == 'Y')" style="margin-left: -12px;">
                    <span data-v-9a6e255c="" class="b-avatar badge-light-success rounded-circle" style="width: 32px; height: 32px;" id="invoice-row-5036">
                        <span class="b-avatar-custom">
                            <svg data-v-9a6e255c="" xmlns="http://www.w3.org/2000/svg" width="14px" height="14px" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-check-circle">
                ...
                <span v-if="(data.item.DSP == 'T')">
                    <span data-v-9a6e255c="" class="b-avatar rounded-circle" style="width: 32px; height: 32px; background-color: orange;" id="invoice-row-5027">
                        <span class="b-avatar-custom">
                            <svg xmlns="http://www.w3.org/2000/svg" width="14px" height="14px" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-file">
                                <path d="M13 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V9z"></path>
                                <polyline points="13 2 13 9 20 9"></polyline>
                ...
                <span v-if="(data.item.DSP == 'Q')">
                    <span data-v-9a6e255c="" class="b-avatar badge-light-success rounded-circle" style="width: 32px; height: 32px;" id="invoice-row-5027">
                        <span class="b-avatar-custom">
                            <svg xmlns="http://www.w3.org/2000/svg" width="14px" height="14px" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-file">
                                <path d="M13 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V9z"></path>
                                <polyline points="13 2 13 9 20 9"></polyline>
                ...
                <span v-if="(data.item.DSP == 'R')">
                    <span data-v-9a6e255c="" class="b-avatar badge-light-warning rounded-circle" style="width: 32px; height: 32px;" id="invoice-row-5031">
                        <span class="b-avatar-custom">
                            <svg xmlns="http://www.w3.org/2000/svg" width="14px" height="14px" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-loader">
                ...
                <span v-if="(data.item.DEL == 'R')">
                    <span data-v-9a6e255c="" class="b-avatar badge-light-danger rounded-circle" style="width: 32px; height: 32px;" id="invoice-row-5027">
                        <span class="b-avatar-custom">
                            <svg xmlns="http://www.w3.org/2000/svg" width="14px" height="14px" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-file">
                                <path d="M13 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V9z"></path>
                                <polyline points="13 2 13 9 20 9"></polyline>
                ...
                <span v-if="(data.item.DEL == 'Y')">
                    <span data-v-9a6e255c="" class="b-avatar badge-light-danger rounded-circle" style="width: 32px; height: 32px;" id="invoice-row-5032">
                        <span class="b-avatar-custom">
                            <svg data-v-9a6e255c="" xmlns="http://www.w3.org/2000/svg" width="14px" height="14px" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-info">
            ...
            <template #cell(actions)="data">
                <div class="text-nowrap">
                    ...
                    <feather-icon :id="`invoice-row-${data.item.id}-preview-icon`" icon="EyeIcon" size="16" class="mx-1" @click="
                            ...
                            " />
                    <b-tooltip title="preview order" :target="`invoice-row-${data.item.id}-preview-icon`" />
                    ...
                    <b-dropdown variant="link" toggle-class="p-0" no-caret :right="$store.state.appConfig.isRTL">
                        ...
                        <b-dropdown-item>
                            ...
                            <span class="align-middle ml-50" :value="data.item.orderNumber" :id="data.item.orderNumber" @click="DownloadBtn($event)">{{ $t('Download')}}</span>
                        </b-dropdown-item>
                        ...
                        <b-dropdown-item>
                            ...
                            <span class="align-middle ml-50" :value="data.item.orderNumber" :id="data.item.orderNumber" @click="DeleteButton($event)">{{ $t('Delete') }}</span>
                        </b-dropdown-item>
                    </b-dropdown>
                </div>
            </template>
    ...
    export default {
        ...
        methods: {
            Cell_table_class(item, type) {
        // if (!item || type !== 'row') return
        if ( item.DSP == "R") return 'table-success'
      },
            // async DeleteButton(event){
            //         // this is corract code //
            //     // let abc  = await axios.delete('https://engine.netsupport.dk:8270/orders/v1/headers/'+ event.target.id)
            // },
            // DownloadBtn(event) {
            // this is corract code //
        //         let AllOrder = ''
        //         this.orderdata.forEach(ele => {
        //             if (event.target.id == ele.orderNumber) {
        //                 AllOrder = ele
        //             }
        //         })
        //         let data =
        //             '\r orderNumber: ' + AllOrder.orderNumber + ' \r\n ' +
                    // 'SHOP: ' + AllOrder.shop + ' \r\n ' +
        //             'buyerName: ' + AllOrder.buyerName + ' \r\n ' +
        //             'buyerCity: ' + AllOrder.buyerCity + ' \r\n ' +
        //             'deliverName: ' + AllOrder.deliverName + ' \r\n ' +
        //             'DELIVERYDATE: ' + AllOrder.deliveryDate + ' \r\n ' +
        //             'deliverCity: ' + AllOrder.deliverCity + ' \r\n ' +
        //             'Netto: ' + AllOrder.Netto + ' \r\n ' 
    
    
        //             const textToBLOB = new Blob([data], { type: 'text/plain' });
        //             const sFileName = `${AllOrder.orderNumber}.txt`;
